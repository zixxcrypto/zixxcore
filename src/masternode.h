...
static const int MASTERNODE_REQUIRED_COINS              = 1000;
...
//
// The Masternode Class. For managing the Darksend process. It contains the input of the MASTERNODE_REQUIRED_COINSXXZ, signature to prove
...
public:
    ...
    /// Is the input associated with collateral public key? (and there is MASTERNODE_REQUIRED_COINS ZIXX - checking if valid masternode)
...
public:
    ...
    static bool Create(std::string strService, std::string strKey, std::string strTxHash, std::string strOutputIndex, std::string strRewardAddres, std::string& strErrorRet, CMasternodeBroadcast &mnbRet, bool fOffline = false);
