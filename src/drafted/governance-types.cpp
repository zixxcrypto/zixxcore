...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "ZixxNetwork") type = ZixxNetwork;
    if(strType == "ZixxNetworkVariable") type = ZixxNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == ZixxNetwork) s = "ZixxNetwork";
    if(type == ZixxNetworkVariable) s = "ZixxNetworkVariable";
