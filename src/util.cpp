...
const char * const BITCOIN_CONF_FILENAME = "zixx.conf";
const char * const BITCOIN_PID_FILENAME = "zixxd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "zixx" is a composite category enabling all Zixx-related debug output
            if(ptrCategory->count(string("zixx"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ZixxCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ZixxCore
    // Mac: ~/Library/Application Support/ZixxCore
    // Unix: ~/.zixx
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ZixxCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ZixxCore";
#else
    // Unix
    return pathRet / ".zixx";
#endif
...
boost::filesystem::path GetTriggerOrderConfigFile()
{
    boost::filesystem::path pathConfigFile(GetArg("-toconf", "triggerorder.conf"));
    if (!pathConfigFile.is_complete()) pathConfigFile = GetDataDir() / pathConfigFile;
    return pathConfigFile;
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty zixx.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override zixx.conf
