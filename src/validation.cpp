...
#if defined(NDEBUG)
# error "Zixx cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in pips) are considered zero fee (for relaying, mining and transaction creation) */
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, CBlockIndex* pindex)
{
    ...
    if (tx.IsCoinBase())
    {
        ...
        // CBlockIndex is not valid when rebuilding the wallet.
        if (pindex
                && pindex->pprev
                && pindex->pprev->nHeight >= consensus.dev_subsidy_start_block
                && pindex->pprev->nHeight < consensus.dev_subsidy_end_block)
        {
            bool subsidy_value_found = false;
            bool subsidy_recipient_found = false;

            // To ensure each subsidy address gets properly rewarded,
            // there has to be an enforcement of payment based on the
            // specific block height.
            auto expected = params.GetDevSubsidyAtHeight(pindex->pprev->nHeight);
            CScript expected_script = GetScriptForDestination(CBitcoinAddress(expected.address).Get());
            CAmount expected_value = ( (consensus.dev_subsidy_percentage / 100.) * (expected.quota / 100.) )
                    * GetBlockSubsidy(pindex->nBits, pindex->pprev->nHeight, consensus);

            CTxOut expected_txout(expected_value, expected_script);
            BOOST_FOREACH(const CTxOut& output, tx.vout) {
                // Valid if the dev subsidy script is correct and if the amount is the expected.
                // Now attempting to find a subsidy reference.

                LogPrint("validation", "Received output: ");
                LogPrint("validation", output.ToString().c_str());
                LogPrint("validation", "\n");
                LogPrint("validation", "Expected output: ");
                LogPrint("validation", expected_txout.ToString().c_str());
                LogPrint("validation", "\n");

                LogPrint("validation", "-----------\n");
                LogPrint("validation", "Received value: ");
                LogPrint("validation", "%i\n", output.nValue);
                LogPrint("validation", "Expected value: ");
                LogPrint("validation", "%i\n", expected_value);

                if (output.scriptPubKey == expected_script) {
                    subsidy_value_found = true;
                    if (output.nValue == expected_value)
                    {
                        subsidy_recipient_found = true;
                        break;
                    }
                }
            }
            ...
            if (!subsidy_recipient_found)
            {
                return state.DoS(100, false, REJECT_DEV_SUBSIDY_MISSING,
                                   "CTransaction::CheckTransaction() : The expected development subsidy is missing");
            }

            // Check if payment has been made.
            if (!subsidy_value_found)
            {
                LogPrint("validation", "Expected value (%i).", expected_value);
                return state.DoS(100, false, REJECT_DEV_SUBSIDY_VALUE_INVALID,
                                   "CTransaction::CheckTransaction() : The expected development subsidy value is incorrect.");
            }
                              ...
                              std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    if (!CheckTransaction(tx, state, chainActive.Tip()) || !ContextualCheckTransaction(tx, state, chainActive.Tip()))
        return false;
...
*/
CAmount GetBlockSubsidy(int nPrevBits, int previous_height, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    if (previous_height == 0) {
        // Initial reward for members.
        return (110000) * COIN;
    }

    // Nominal reward associated with a block.
    CAmount subsidy = 5 * COIN;

    // 200 blocks - 0 reward => to ensure the coins are mature.
    // 1440 blocks - 0.05 reward (1%) - instamine
    if (previous_height < 201) {
        return 0;
    }

    if (previous_height < 1441) {
        return (subsidy / 100.);
    }

    // From here on, we decrease until we reach one XZX
    // Otherwise it's the nominal value, which declines yearly by 10%
    for (int i = consensusParams.nSubsidyHalvingInterval; i <= previous_height; i += consensusParams.nSubsidyHalvingInterval) {
        subsidy -= subsidy/10;
    }

    subsidy = std::max(subsidy, COIN);
...
{
    CAmount ret = (8. * blockValue) / 9.; // start at 20%, but we've already subtracted 10% before
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    if (!CheckBlock(block, state, pindex, !fJustCheck, !fJustCheck))
        return false;
    ...
    // ZIXX : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(ZIXX): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(ZIXX): couldn't find masternode or superblock payments"),
...
bool static FlushStateToDisk(CValidationState &state, FlushStateMode mode) {
    ...
    uint64_t cacheSize = pcoinsTip->DynamicMemoryUsage() * DB_PEAK_USAGE_FACTOR;
...
bool CheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* pIndex, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // ZIXX : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(ZIXX): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(ZIXX): spork is off, skipping transaction locking checks\n");
    }
    ...
    // Check transactions
    BOOST_FOREACH(const CTransaction& tx, block.vtx)
        if (!CheckTransaction(tx, state, pIndex))
...
static bool AcceptBlock(const CBlock& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock)
{
    ...
    if ((!CheckBlock(block, state, pindex)) || !ContextualCheckBlock(block, state, pindex->pprev)) {
...
bool TestBlockValidity(CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (!CheckBlock(block, state, &indexDummy, fCheckPOW, fCheckMerkleRoot))
        return false;
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        if (nCheckLevel >= 1 && !CheckBlock(block, state, pindex))
