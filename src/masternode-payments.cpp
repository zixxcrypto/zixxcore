...
*
*   Why is this needed?
*   - In Zixx some blocks are superblocks, which output much higher amounts of coins
...
int CMasternodePayments::GetMinMasternodePaymentsProto() {
    if(sporkManager.IsSporkActive(SPORK_97_MIN_VERSION_WATERMARK)) return MIN_MASTERNODE_PAYMENT_PROTO_VERSION_3;
...
{
    if(fLiteMode) return; // disable all Zixx specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f ZIXX\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
