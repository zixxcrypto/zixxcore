...
public:
    TxViewDelegate(const PlatformStyle *_platformStyle, QObject *parent=nullptr):
        QAbstractItemDelegate(parent), unit(BitcoinUnits::XZX),
        platformStyle(_platformStyle)
    {
        _font = GUIUtil::defaultUIFont();
    }
                      ...
                      const QModelIndex &index ) const
    {
        ...
        if(!confirmed)
        {
            amountText = amountText + " - " + tr("pending");
        }

        painter->drawText(amountRect, Qt::AlignRight|Qt::AlignVCenter, amountText);
        painter->setPen(pen);
        QFont local_font = painter->font();
        local_font.setWeight( QFont::Bold );
        painter->setFont(local_font);
        painter->drawText(amountRect, Qt::AlignLeft|Qt::AlignVCenter, GUIUtil::dateTimeStr(date));
        local_font.setWeight( QFont::Normal );
    ...
    , txdelegate(new TxViewDelegate(platformStyle, this))
{
    ...
    // Adding a nice icon to the info button.

    ui->privateSendInfo->setIcon(QIcon(":/icons/" + theme + "/info"));
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    {
        // update the display unit, to not use the default ("ZIXX")
        updateDisplayUnit();
