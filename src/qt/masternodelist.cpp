...
void MasternodeList::StartAlias(std::string strAlias)
{
    ...
    BOOST_FOREACH(CMasternodeConfig::CMasternodeEntry mne, masternodeConfig.getEntries()) {
        if(mne.getAlias() == strAlias) {
            ...
            bool fSuccess = CMasternodeBroadcast::Create(mne.getIp(), mne.getPrivKey(), mne.getTxHash(), mne.getOutputIndex(), mne.getRewardAddress(), strError, mnb);
...
void MasternodeList::on_CreateMasternode_clicked()
{
    // Wizard:
    //    1) Ask for a masternode name, check it is unique amounst the other alias's.
    //    2) generate a new receiving address using that alias, and send 1000 coins to it(edited)
    //          -
    //    3) generate a masternode genkey
    //          - masternode genkey
    //    4) ask for (or detect automagically?) the valid ip of the windows machine
    //    5) show a display of how many blocks have passed since the 1000 coins was sent. Once it hits 15 confirmations, go to the next stage
    //    6) populate the masternode.conf file, restart the wallet, and start the node
    if (!walletModel)
    {
        LogPrint("masternode", "Unable to run masternode wizard. WalletModel is invalid for some reason.");
        return;
    }

    MasternodeCreateDialog *dialog = new MasternodeCreateDialog(this);
