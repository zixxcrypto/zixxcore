...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns a new Zixx address for receiving payments.\n"
            "If 'account' is specified (DEPRECATED), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) DEPRECATED. The account name for the address to be linked to. If not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"zixxaddress\"    (string) The new zixx address\n"
            "\nExamples:\n"
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nDEPRECATED. Returns the current Zixx address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"zixxaddress\"   (string) The account zixx address\n"
            "\nExamples:\n"
...
UniValue getrawchangeaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new Zixx address, for receiving change.\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount \"zixxaddress\" \"account\"\n"
            "\nDEPRECATED. Sets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"zixxaddress\"  (string, required) The zixx address to be associated with an account.\n"
    ...
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Zixx address");
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount \"zixxaddress\"\n"
            "\nDEPRECATED. Returns the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"zixxaddress\"  (string, required) The zixx address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"zixxaddress\"  (string) a zixx address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 7)
        throw runtime_error(
            "sendtoaddress \"zixxaddress\" amount ( \"comment\" \"comment-to\" subtractfeefromamount use_is use_ps )\n"
            "\nSend an amount to a given address.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"zixxaddress\" (string, required) The zixx address to send to.\n"
            ...
            "                             The recipient will receive less amount of Zixx than you enter in the amount field.\n"
...
UniValue instantsendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "instantsendtoaddress \"zixxaddress\" amount ( \"comment\" \"comment-to\" subtractfeefromamount )\n"
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"zixxaddress\",     (string) The zixx address\n"
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"zixxaddress\" \"message\"\n"
            "\nSign a message with the private key of an address"
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"zixxaddress\"  (string, required) The zixx address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "getreceivedbyaddress \"zixxaddress\" ( minconf addlockconf )\n"
            "\nReturns the total amount received by the given zixxaddress in transactions with specified minimum number of confirmations.\n"
            "\nArguments:\n"
            "1. \"zixxaddress\"  (string, required) The zixx address for transactions.\n"
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"tozixxaddress\" amount ( minconf addlockconf \"comment\" \"comment-to\" )\n"
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a zixx address."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"    (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"tozixxaddress\"  (string, required) The zixx address to send funds to.\n"
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 8)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount     (numeric or string) The zixx address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "                           Those recipients will receive less zixxs than you enter in their corresponding amount field.\n"
    ...
    BOOST_FOREACH(const string& name_, keys)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Zixx address: ")+name_);
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a Zixx address or hex-encoded public key.\n"
            "If 'account' is specified (DEPRECATED), assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of zixx addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) zixx address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) DEPRECATED. An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"zixxaddress\"  (string) A zixx address associated with the keys.\n"
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"zixxaddress\",  (string) The zixx address of the transaction. Not present for \n"
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp)
        throw runtime_error(
            ...
            "    \"address\":\"zixxaddress\",  (string) The zixx address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "      \"address\" : \"zixxaddress\",      (string) The zixx address involved in the transaction\n"
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    ...
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending zixxs\n"
            "\nArguments:\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending zixx\n"
            + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n"
            + HelpExampleCli("signmessage", "\"zixxaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "Wallet encrypted; Zixx server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending zixxs.\n"
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "3. \"addresses\"      (string) A json array of zixx addresses to filter\n"
            "    [\n"
            "      \"address\"     (string) zixx address\n"
            "      ,...\n"
            "    ]\n"
            "\nResult\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the zixx address\n"
    ...
    if (params.size() > 2) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Zixx address: ")+input.get_str());
            if (setAddress.count(address))
...
// ZIXX Orders
UniValue ordercreatetrigger(const UniValue& params, bool fHelp)
{
    if (!EnsureWalletIsAvailable(fHelp))
        return NullUniValue;

    // createorder [unique label] [from address] [percentage] [to address]
    if (fHelp || params.size() < 4)
        throw runtime_error(
            "ordercreatetrigger \"label\" \"from_adress\" percentage \"to_address\" \n"
            "\nCreates a standing order to transfer a percentage of the income when a transaction arrives at that address.\n"
            "\nArguments:\n"
            "1. \"label\"           (string,  required) A friendly name to this standing order.\n"
            "2. \"from_adress\"     (string,  required) The source address in which this standing order will operate.\n"
            "3. percentage          (double,  required) Percentage (0 to 100, fractional) of the income to be paid."
            "4. \"to_adress\"       (string,  required) The payee address, where the funds will be transferred to.\n"
            "5. flush               (boolean, optional) Whether to write to the internal DB instantly. Defaults to true\n"
            "\nExamples:\n"
            "\nSend 60% of the rewards to another address\n"
            + HelpExampleCli("ordercreatetrigger", "\"alimony_second_child\" zDHKLRf2JZDxvXEuHbM9qevFVZeSkSk2gW 50 zET3frUzt2UDRN7LsPTk2isXR2kX4DpnHP")
        );
    ...
    if (alias.empty())
    {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Provided value for alias is invalid.");
    }

    CBitcoinAddress from_address(params[1].get_str());
    if (!from_address.IsValid())
    {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid ZIXX source address");
    }

    if (!IsMine(*pwalletMain, from_address.Get()))
    {
        throw JSONRPCError(RPC_MISC_ERROR, "Trigger order requires from address to be your own address");
    }

    double percentage = RealFromValue(params[2]);
    if (percentage < 0 || percentage > 100)
    {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Provided value for percentage is invalid. Valid range: (0-100].");
    }

    CBitcoinAddress dest_address(params[3].get_str());
    if (!dest_address.IsValid())
    {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid ZIXX dest address");
    }

    bool flush = true;
    if (params.size() > 4) {
        flush = params[4].get_bool();
    }

    // Provided params seem to be OK, add it to the DB.
    triggerOrderConfig.add(alias, from_address.ToString(), percentage, dest_address.ToString());

    if (flush) {
        triggerOrderConfig.write();
    }

    return UniValue(true);
}

UniValue orderlisttrigger(const UniValue &params, bool fHelp)
{
    if (!EnsureWalletIsAvailable(fHelp))
        return NullUniValue;
    ...
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "listtriggerorder\n"
            "\nLists the currently registered trigger orders\n"
            "\nExamples:\n"
            "\nlisttriggerorder\n"
            + HelpExampleCli("listtriggerorder", "")
        );
    ...
    std::vector<TriggerOrderConfig::TriggerOrderEntry> entries = triggerOrderConfig.getEntries();
    for (std::vector<TriggerOrderConfig::TriggerOrderEntry>::iterator it = entries.begin() ; it != entries.end(); ++it) {
        UniValue obj(UniValue::VOBJ);
        obj.push_back(Pair("alias",         it->getAlias()));
        obj.push_back(Pair("from_address",  it->getFromAddress()));
        obj.push_back(Pair("percentage",    it->getPercentage()));
        obj.push_back(Pair("to_address",    it->getToAddress()));;
        ret.push_back(obj);
    }
    return ret;
}

UniValue orderdeletetrigger(const UniValue &params, bool fHelp)
{
    if (!EnsureWalletIsAvailable(fHelp))
        return NullUniValue;

    // deletetriggerorder [unique label]
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "deletetriggerorder\n"
            "\nDeletes a trigger order specified by the alias\n"
            "\nExamples:\n"
            "\ndeletetriggerorder \"alimony_fourth_child\"\n"
            + HelpExampleCli("deletetriggerorder", "\"alimony_fourth_child\"")
        );

    LOCK2(cs_main, pwalletMain->cs_wallet);

    bool ret = triggerOrderConfig.remove(params[0].get_str());
    triggerOrderConfig.write();
...
UniValue getqueuedtriggers(const UniValue &params, bool fHelp)
{
    if (!EnsureWalletIsAvailable(fHelp))
        return NullUniValue;
    ...
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "getqueuedtriggers\n"
            "\nLists the currently queued trigger orders\n"
            "\nExamples:\n"
            "\ngetqueuedtriggers\n"
            + HelpExampleCli("getqueuedtriggers", "")
        );
    ...
    std::vector<TriggerTransactionManager::TriggerTransactionEntry> entries = ttManager.getEntries();
    for (std::vector<TriggerTransactionManager::TriggerTransactionEntry>::iterator it = entries.begin() ; it != entries.end(); ++it) {
        UniValue obj(UniValue::VOBJ);
        obj.push_back(Pair("alias",         it->getAlias()));
        obj.push_back(Pair("from_address",  it->getFromAddress().ToString()));
        obj.push_back(Pair("value",         it->getValue()));
        obj.push_back(Pair("to_address",    it->getToAddress().ToString()));
        obj.push_back(Pair("input_hash",    it->getInputHash().ToString()));
        obj.push_back(Pair("committed",     it->getCommitted()));
        ret.push_back(obj);
    }
    return ret;
}
...
UniValue getvalue(const UniValue &params, bool fHelp)
{
    if (!EnsureWalletIsAvailable(fHelp))
        return NullUniValue;
    ...
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "getvalue\n"
            "\nReturns the current value of your portfolio in different currencies\n"
        );

    LOCK2(cs_main, pwalletMain->cs_wallet);
    CAmount balance = pwalletMain->GetBalance() / COIN;

    boost::system::error_code ec;
    boost::asio::io_service service;
    boost::asio::ssl::context context(boost::asio::ssl::context::method::sslv23_client);
    boost::asio::ssl::stream<boost::asio::ip::tcp::socket> ssock(service, context);
    boost::asio::ip::tcp::resolver resolver(service);
    auto it = resolver.resolve({"api.zixx.org", "443"});
    ...
    request_stream << "GET /extended/value/" <<balance << " HTTP/1.1\r\n";
    request_stream << "Host: api.zixx.org\r\n";
    ...
    std::vector<std::string> vec_response;
    boost::split(vec_response, response, boost::is_any_of("\n\r"));

    // Everything looks fine.
    UniValue ret(UniValue::VSTR);
    ret = vec_response.back();
    return ret;
}
